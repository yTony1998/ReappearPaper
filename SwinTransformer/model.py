...
class WindowAttention(nn.Module):
    def __init__(self, dim, heads, head_dim, shifted, window_size, relative_pos_embedding):
        ...
        if self.relative_pos_embedding:
            ...
            self.pos_embedding = nn.Parameter(torch.randn(2*window_size, 2*window_size))
    ...
    def forward(self, x):
        ...
        #swin-transformer 精华所在
        if self.shifted:
            dots[:, :, -nw_w:] += self.upper_lower_mask #b h w i=nw_h*nw_w j
...
class SwinBlock(nn.Module):
    def __init__(self, dim, heads, head_dim, mlp_dim, shifted, window_size, relative_pos_embedding):
        super().__init__()
        self.attention_block = Residual(PreNorm(dim, WindowAttention(dim=dim, heads=heads, head_dim=head_dim, shifted = shifted, window_size=window_size,
